...
const headCheck = document.querySelector("input#headCheck")
TODOS_KEY = "todos"
...
function checkedListHandler(event) {
...
function paintToDo(newTodo) {
  ...
  ch.type = "checkbox"
  ch.addEventListener('change', checkedListHandler)
...
function checkBoxHandler() {
  ...
  for (let i = 0; i < checkBoxes.length; i++) {
    checkBoxes[i].checked = headCheck.checked
    checkedList(checkBoxes[i].parentElement.parentElement)
  }
  let dat = new Date(1739344210076)
  console.log(dat.getDate());
  cconsole.log(Date.parse(dat));
  console.log(checkBoxes);
}

toDoForm.addEventListener("submit", handleToDoSubmit);
headCheck.addEventListener("change", checkBoxHandler)
